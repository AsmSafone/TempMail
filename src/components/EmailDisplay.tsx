import { Copy, RefreshCw, Trash2 } from 'lucide-react';

interface EmailDisplayProps {
  email: string;
  onCopy: () => void;
  onRefresh: () => void;
  onDelete: () => void;
}

export const EmailDisplay = ({ email, onCopy, onRefresh, onDelete }: EmailDisplayProps) => {
  return (
    <div className="bg-black border-2 border-green-500 rounded-lg p-6 shadow-2xl shadow-green-500/50">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-green-400 font-mono text-lg">Your Temporary Email</h2>
        <div className="flex gap-2">
          <button
            onClick={onRefresh}
            className="p-2 bg-green-900/30 hover:bg-green-900/50 border border-green-500 rounded text-green-400 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/50"
            title="Generate new email"
          >
            <RefreshCw size={18} />
          </button>
          <button
            onClick={onDelete}
            className="p-2 bg-red-900/30 hover:bg-red-900/50 border border-red-500 rounded text-red-400 transition-all duration-300 hover:shadow-lg hover:shadow-red-500/50"
            title="Delete mailbox"
          >
            <Trash2 size={18} />
          </button>
        </div>
      </div>

      <div className="flex items-center gap-3 bg-green-950/30 border border-green-700 rounded p-4">
        <code className="flex-1 text-green-300 font-mono text-lg break-all">
          {email}
        </code>
        <button
          onClick={onCopy}
          className="p-2 bg-green-900/50 hover:bg-green-900/70 border border-green-500 rounded text-green-400 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/50 flex-shrink-0"
          title="Copy to clipboard"
        >
          <Copy size={18} />
        </button>
      </div>

      <div className="mt-4 text-green-500/70 text-sm font-mono text-center animate-pulse">
        &gt; SECURE TEMPORARY MAILBOX ACTIVE &lt;
      </div>
    </div>
  );
};
